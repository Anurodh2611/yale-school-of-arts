    ...
    return (
        <div className="flex-1 flex items-center justify-center p-4">
            <div className="w-full max-w-md bg-[var(--color-surface)] border-4 border-[var(--color-border)] brutal-shadow p-8">
                <h2 className="text-4xl font-black uppercase tracking-tighter mb-6 text-center text-[var(--color-text-primary)]">Register</h2>
                {error && (
                ...
                <form onSubmit={handleSubmit} className="flex flex-col space-y-4">
                    <div className="flex flex-col space-y-1">
                        <label className="font-bold uppercase text-lg text-[var(--color-text-primary)]">Username</label>
                        <input
                            type="text"
                            required
                            value={username}
                ...
                <div className="mt-6 text-center font-bold text-lg text-[var(--color-text-primary)] uppercase">
                    Got an account? <Link to="/login" className="text-[var(--color-primary)] hover:underline ml-1 hover:text-blue-600 transition-colors">Login</Link>
                </div>
            </div>
        </div>
    );
